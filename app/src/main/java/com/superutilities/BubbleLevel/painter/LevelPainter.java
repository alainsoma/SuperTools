package com.superutilities.BubbleLevel.painter;
...
public class LevelPainter implements Runnable {
    ...
    public LevelPainter(SurfaceHolder surfaceHolder, Context context, 
        ...
        // drawable
        this.level1D = ContextCompat.getDrawable(context, R.drawable.level_1d); //context.getResources().getDrawable(R.drawable.level_1d);
		...
		if (!(lockEnabled && locked) || !initialized) {
	        synchronized (this.surfaceHolder) {
	    		...
	    		switch (newOrientation) {
		    		case LEFT :		// left_side_header_layout
		    		case RIGHT :	// right
		    			height = canvasWidth;
		    			width = canvasHeight;
